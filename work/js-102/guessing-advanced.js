...
function pickGuess( wordInfo, history ) {
  ...
  // if we guessed a wrong letter we remove that from our guesslist
  if( previousResult.similar === history.info.correctLetters.length ){
      wordInfo.magic = wordInfo.magic.replace( previousResult.guess.slice(-1) , '');
      history.info.letterIndex -= 1;
  }
