...
const guessesHistory = {
    results : [], // stores previous guessed words and respective commonLetters
    noOfTurnsTaken : 0
};

let result = {}; // stores guessed word and its common letters
...
function getGuessedWord() {
    //returns the word typed by user
    return document.querySelector('#guess-input').value.toUpperCase();
}
...
function addAddListener() {
    //adding required event listeners to the input and button fields
    document.querySelector('#guess-input').addEventListener('input', guessButtonDisabilityToggle);
    document.querySelector('#guess-button').addEventListener('click', evaluateGuessedWord);
    document.querySelector('#guess-input').addEventListener('keypress', event => {if( event.keyCode === 13 ) evaluateGuessedWord(); });
}

function guessButtonDisabilityToggle() {
    // toggles the guess button between enabled and disabled
...
function evaluateGuessedWord() {
    //evaluates the guessed word and stores the result for future use
    const valid = validateInput();
    ...
    // forms a result from the input guessedWord and stores in history.
    result = {};
    result.word = getGuessedWord();
    result.noOfCommonLetters = countCommonLetters( wordInfo.word , result.word );

    guessesHistory.results.push( result );
    guessesHistory.noOfTurnsTaken += 1;
    render();
    if( result.word === wordInfo.word ){
        renderWinAndAskForReset();
    }
}

function validateInput() {
    // checks the validity of the typed in word
...
function displayError() {
    ...
    document.querySelector('#guess-input').setCustomValidity('invalid');
}

function render() {
    //renders the page
    document.querySelector('.score').innerHTML = guessesHistory.noOfTurnsTaken;
    document.querySelector('#guess-input').value = '';
    document.querySelector('#guess-input').setCustomValidity('');
    document.querySelector('#guess-button').setAttribute('disabled', '');
    document.querySelector('.errors').classList.remove('show');
    document.querySelector('.previous-guesses').innerHTML = generatePreviousGuessesList();
    setThePreviousGuessesScrollToBottom();
}

function generatePreviousGuessesList() {
    // generates the previous guessed list and respective count of
    // CommonLetters to dispay
    const previousGuessesList = guessesHistory.results.map(( guess ) => `<li>${guess.word} | ${guess.noOfCommonLetters} letters in common </li>` ).join('\n');
    return previousGuessesList;
}

function setThePreviousGuessesScrollToBottom() {
    // sets the scroll bar of the previous guesses to display
    // the last or latest element
    const guessList = document.querySelector('.previous-guesses');
    guessList.scrollTop = guessList.scrollHeight;
}

function renderWinAndAskForReset() {
    // renders the win condition and asks the user to reset
    displayWin();
    createResetButton();
}
...
function displayWin() {
    ...
    document.querySelector('.user-win').classList.add('show');
    document.querySelector('#guess-input').classList.add('hide');
}
function createResetButton() {
    // changes the guess button to reset button
    const guessButton = document.querySelector('#guess-button');
    guessButton.innerHTML = 'Reset';
    guessButton.classList.add('reset-button');
    guessButton.removeAttribute('disabled');
    guessButton.addEventListener('click', resetGame);

}
function resetGame() {
    //reseting win display
    document.querySelector('.user-win').classList.remove('show');
    document.querySelector('#guess-input').classList.remove('hide');

    //resetting to new word
    wordInfo.word = pickWord();

    //resetting to new history;
    guessesHistory.results = [];
    guessesHistory.noOfTurnsTaken = 0;

    //changing the reset button to guess button
    const guessButton = document.querySelector('#guess-button');
    guessButton.innerHTML = 'Guess';
    guessButton.classList.remove('reset-button');
    guessButton.removeEventListener('click', resetGame);
    guessButton.addEventListener('click', evaluateGuessedWord);
    render();
}
